    ...
    <DrawerPortal>
      ...
      <DrawerViewport position={position} variant={variant}>
        <DrawerPrimitive.Popup
          className={(state) =>
            cn(
              "relative flex max-h-full min-h-0 w-full min-w-0 flex-col bg-slate-900 not-dark:bg-clip-padding text-slate-100 shadow-2xl outline-none transition-[transform,box-shadow,height,background-color] duration-450 ease-[cubic-bezier(0.32,0.72,0,1)] will-change-transform [--peek:calc(--spacing(6)-1px)] [--scale-base:calc(max(0,1-(var(--nested-drawers)*var(--stack-step))))] [--scale:clamp(0,calc(var(--scale-base)+(var(--stack-step)*var(--stack-progress))),1)] [--shrink:calc(1-var(--scale))] [--stack-peek-offset:max(0px,calc((var(--nested-drawers)-var(--stack-progress))*var(--peek)))] [--stack-progress:clamp(0,var(--drawer-swipe-progress),1)] [--stack-step:0.05] before:pointer-events-none before:absolute before:inset-0 before:shadow-[0_1px_--theme(--color-black/4%)] after:pointer-events-none after:absolute after:bg-slate-900 data-swiping:select-none data-nested-drawer-open:overflow-hidden data-nested-drawer-open:bg-[color-mix(in_srgb,var(--popover),var(--color-black)_calc(2%*(var(--nested-drawers)-var(--stack-progress))))] data-ending-style:shadow-transparent data-starting-style:shadow-transparent data-ending-style:duration-[calc(var(--drawer-swipe-strength)*400ms)] dark:data-nested-drawer-open:bg-[color-mix(in_srgb,var(--popover),var(--color-black)_calc(6%*(var(--nested-drawers)-var(--stack-progress))))] dark:before:shadow-[0_-1px_--theme(--color-white/6%)] border border-slate-800",
              "touch-none",
              position === "bottom" &&
      ...
      render={
        <div
          className={cn(
            ...
            variant === "default" &&
              "border-t border-slate-800 bg-slate-900/80 pt-4 pb-[calc(env(safe-area-inset-bottom,0px)+--spacing(4))]",
            variant === "bare" &&
  ...
  const content = (
    <DrawerPrimitive.Content
      render={
        <div
          className={cn(
            "p-6 in-[[data-slot=drawer-popup]:has([data-slot=drawer-header])]:pt-1 in-[[data-slot=drawer-popup]:has([data-slot=drawer-footer]:not(.border-t))]:pb-1 text-slate-300",
            className,
          )}
...
export function DrawerBar({
  ...
  return (
    <div
      aria-hidden
      className={cn(
        "absolute flex touch-none items-center justify-center p-3 before:rounded-full before:bg-slate-700",
        horizontal
...
export function DrawerMenuItem({
  ...
  return (
    <button
      className={cn(
        "flex min-h-9 w-full cursor-default select-none items-center gap-2 rounded-sm px-2 py-1 text-base text-slate-200 outline-none hover:bg-slate-800 hover:text-white disabled:pointer-events-none disabled:opacity-64 data-[variant=destructive]:text-red-400 sm:min-h-8 sm:text-sm [&>svg:not([class*='opacity-'])]:opacity-80 [&>svg:not([class*='size-'])]:size-4.5 sm:[&>svg:not([class*='size-'])]:size-4 [&>svg]:pointer-events-none [&>svg]:-mx-0.5 [&>svg]:shrink-0",
        className,
      )}
...
export function DrawerMenuSeparator({
  ...
  return (
    <div
      className={cn("mx-2 my-1 h-px bg-slate-800", className)}
...
export function DrawerMenuGroupLabel({
  ...
  return (
    <div
      className={cn(
        "px-2 py-1.5 font-medium text-slate-400 text-xs",
        className,
      )}
...
export function DrawerMenuTrigger({
  ...
  return (
    <DrawerTrigger
      className={(state) =>
        cn(
          "flex min-h-9 w-full cursor-default select-none items-center gap-2 rounded-sm px-2 py-1 text-base text-slate-200 outline-none hover:bg-slate-800 hover:text-white sm:min-h-8 sm:text-sm [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
          typeof className === "function" ? className(state) : className,
        )
      }
...
export function DrawerMenuCheckboxItem({
  ...
  return (
    <CheckboxPrimitive.Root
      ...
      className={(state) =>
        cn(
          "grid min-h-9 w-full cursor-default select-none items-center gap-2 rounded-sm px-2 py-1 text-base text-slate-200 outline-none hover:bg-slate-800 hover:text-white data-disabled:pointer-events-none data-disabled:opacity-64 sm:min-h-8 sm:text-sm [&_svg:not([class*='opacity-'])]:opacity-80 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:-mx-0.5 [&_svg]:shrink-0",
          variant === "switch"
...
export function DrawerMenuRadioItem({
  ...
  return (
    <RadioPrimitive.Root
      ...
      {...props}
    >
      <RadioPrimitive.Indicator className="col-start-1 text-blue-400">
        <svg
          fill="none"
          height="24"
          stroke="currentColor"
